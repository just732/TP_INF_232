...
st.markdown("""
    ...
    /* LES TROIS BULLES (CARTES BLEUES DE L'IMAGE 2) */
...
# --- BASE DE DONNÉES ---
def get_connection(): return sqlite3.connect('audit_v17_final.db', check_same_thread=False)
def init_db():
    conn = get_connection()
    c = conn.cursor()
...
if st.session_state.page == "Accueil":
    ...
    with col_a:
        st.markdown('<div class="info-card"><h4>Hôpitaux Publics</h4><p>172 établissements audités au Cameroun.</p></div>', unsafe_allow_html=True)
    with col_b:
    ...
    # LES TROIS BULLES (REPRODUCTION IMAGE 2 SANS BOUTONS ROUGES)
...
# --- PAGE 3 : ADMIN ---
    ...
    t1, t2 = st.tabs(["👤 Identification", "📊 Résultats"])
    with t1:
...
# --- PAGE 4 : INFOS ---
    ...
    st.button("RETOUR", on_click=lambda: changer_page("Accueil"))
    st.markdown('</div>', unsafe_allow_html=True)
    st.markdown("<div class='white-box'><h3>Infos Projet</h3><p>Patient Plus centralise les données pour la réforme hospitalière camerounaise.</p></div>", unsafe_allow_html=True)
